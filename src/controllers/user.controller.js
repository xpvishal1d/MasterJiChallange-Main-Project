...
const logOutUser = asyncHandler(async (req, res) => {
  ...
  await User.findByIdAndUpdate(
    userid,
    {
      $unset: { refreshToken: "" }, // better than undefined
    },
    { new: true }
  );
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  if (!avatar.url) {
    throw new ApiError(500, "Error whilr uploading on avatar");
  }
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  if (!coverImage.url) {
    throw new ApiError(500, "Error whilr uploading on cover image");
  }
